package com.tw ter.fr gate.pushserv ce.model
 ...
 mport com.tw ter.fr gate.pushserv ce.model. b s.Sc duledSpaceSubscr ber b s2Hydrator
 mport com.tw ter.fr gate.pushserv ce.model.ntab.Sc duledSpaceSubscr berNTabRequestHydrator
...
class Sc duledSpaceSubscr berPushCand date(
  ...
  pushModelScorer: PushMLModelScorer)
    ...
    w h Sc duledSpaceSubscr berCand date
    w h SpaceCand dateFanoutDeta ls
    w h Sc duledSpaceSubscr ber b s2Hydrator
    w h Sc duledSpaceSubscr berNTabRequestHydrator {
  ...
  overr de val statsRece ver: StatsRece ver =
    statsScoped.scope("Sc duledSpaceSubscr berCand date")
}

case class Sc duledSpaceSubscr berCand datePred cates(conf g: Conf g)
    extends Bas cSendHandlerPred cates[Sc duledSpaceSubscr berPushCand date] {
  ...
  overr de val preCand dateSpec f cPred cates: L st[
    Na dPred cate[Sc duledSpaceSubscr berPushCand date]
  ] =
    L st(
      Pred catesForCand date.paramPred cate(FeatureSw chParams.EnableSc duledSpaceSubscr bers),
      SpacePred cate.narrowCastSpace,
      SpacePred cate.target nSpace(conf g.aud oSpacePart c pantsStore),
      SpacePred cate.spaceHostTargetUserBlock ng(conf g.edgeStore),
      Pred catesForCand date.dupl cateSpacesPred cate
    )
}

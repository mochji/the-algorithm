...
object KafkaProcessorUserMod f cat onModule extends Tw terModule w h Logg ng {
  ...
  def prov desKafkaProcessor(
  ...
  ): AtLeastOnceProcessor[UnKeyed, UserMod f cat on] = {
    KafkaProcessorProv der.prov deDefaultAtLeastOnceProcessor(
      ...
      s ceValueDeser al zer = NullableScalaSerdes
        .Thr ft[UserMod f cat on](statsRece ver.counter("deser al zerErrors")).deser al zer,
      ...
      adapter = new UserMod f cat onAdapter,

...
object KafkaProcessorEma lNot f cat onEventModule extends Tw terModule w h Logg ng {
  overr de def modules = Seq(FlagsModule)
  pr vate val not f cat onEventAdapter = new Ema lNot f cat onEventAdapter
  ...
  def prov desKafkaProcessor(
  ...
  ): AtLeastOnceProcessor[UnKeyed, Not f cat onScr be] = {
    KafkaProcessorProv der.prov deDefaultAtLeastOnceProcessor(
      ...
      s ceValueDeser al zer = NullableScalaSerdes
        .Thr ft[Not f cat onScr be](statsRece ver.counter("deser al zerErrors")).deser al zer,
      ...
      adapter = not f cat onEventAdapter,
      ...
      maybeProcess = ZoneF lter ng.localDCF lter ng
    )
  }
}

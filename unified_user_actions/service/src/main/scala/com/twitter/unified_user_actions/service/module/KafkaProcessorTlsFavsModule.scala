...
object KafkaProcessorTlsFavsModule extends Tw terModule w h Logg ng {
  overr de def modules = Seq(FlagsModule)

  pr vate val tlsFavsAdapter = new TlsFavsAdapter
  ...
  def prov desKafkaProcessor(
  ...
  ): AtLeastOnceProcessor[UnKeyed, Contextual zedFavor eEvent] = {
    KafkaProcessorProv der.prov deDefaultAtLeastOnceProcessor(
      ...
      s ceValueDeser al zer = NullableScalaSerdes
        .Thr ft[Contextual zedFavor eEvent](
          statsRece ver.counter("deser al zerErrors")).deser al zer,

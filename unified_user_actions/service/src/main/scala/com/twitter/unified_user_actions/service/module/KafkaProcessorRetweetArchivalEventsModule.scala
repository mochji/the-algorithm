...
object KafkaProcessorRet etArch valEventsModule extends Tw terModule w h Logg ng {
  overr de def modules = Seq(FlagsModule)

  pr vate val adapter = new Ret etArch valEventsAdapter
  ...
  def prov desKafkaProcessor(
  ...
  ): AtLeastOnceProcessor[UnKeyed, Ret etArch valEvent] = {
    KafkaProcessorProv der.prov deDefaultAtLeastOnceProcessor(
      ...
      s ceValueDeser al zer = NullableScalaSerdes
        .Thr ft[Ret etArch valEvent](statsRece ver.counter("deser al zerErrors")).deser al zer,

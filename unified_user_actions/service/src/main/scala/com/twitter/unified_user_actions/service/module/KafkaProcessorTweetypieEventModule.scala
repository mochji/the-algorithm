...
object KafkaProcessorT etyp eEventModule extends Tw terModule w h Logg ng {
  overr de def modules: Seq[ nject.Module] = Seq(FlagsModule)

  pr vate val t etyp eEventAdapter = new T etyp eEventAdapter
  ...
  def prov desKafkaProcessor(
  ...
  ): AtLeastOnceProcessor[UnKeyed, T etEvent] = {
    KafkaProcessorProv der.prov deDefaultAtLeastOnceProcessor(
      ...
      s ceValueDeser al zer = NullableScalaSerdes
        .Thr ft[T etEvent](statsRece ver.counter("deser al zerErrors")).deser al zer,

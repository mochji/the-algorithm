...
@S ngleton
class WhoToFollowCand dateP pel neConf gBu lder @ nject() (
  whoToFollowCand dateS ce: PeopleD scoveryCand dateS ce) {

  /**
   * Bu ld a WhoToFollowCand dateP pel neConf g
   *
   * To create a DependentCand dateP pel neConf g  nstead see [[WhoToFollowDependentCand dateP pel neConf gBu lder]].
   *
  ...
  ): WhoToFollowCand dateP pel neConf g[Query] =
    new WhoToFollowCand dateP pel neConf g(
       dent f er =  dent f er,

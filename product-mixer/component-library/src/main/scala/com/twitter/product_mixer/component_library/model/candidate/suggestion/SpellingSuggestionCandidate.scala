package com.tw ter.product_m xer.component_l brary.model.cand date.suggest on
 ...
 mport com.tw ter.product_m xer.core.model.marshall ng.response.urt. em.suggest on.Spell ngAct onType
 mport com.tw ter.product_m xer.core.model.marshall ng.response.urt. em.suggest on.TextResult

/**
 * Canon cal Spell ngSuggest onCand date model. Always prefer t  vers on over all ot r var ants.
 *
 ...
 */
f nal class Spell ngSuggest onCand date pr vate (
  overr de val  d: Str ng,
  val textResult: TextResult,
  val spell ngAct onType: Opt on[Spell ngAct onType],
  val or g nalQuery: Opt on[Str ng])
    extends Un versalNoun[Str ng] {

  /**
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[Spell ngSuggest onCand date]
  ...
  overr de def equals(that: Any): Boolean =
    that match {
      case cand date: Spell ngSuggest onCand date =>
        (
          (t  eq cand date)
            || ((hashCode == cand date.hashCode)
              && ( d == cand date. d && textResult == cand date.textResult && spell ngAct onType == cand date.spell ngAct onType && or g nalQuery == cand date.or g nalQuery))
        )
      case _ =>
        false
    }
...
object Spell ngSuggest onCand date {
  def apply(
     d: Str ng,
    textResult: TextResult,
    spell ngAct onType: Opt on[Spell ngAct onType],
    or g nalQuery: Opt on[Str ng]
  ): Spell ngSuggest onCand date =
    new Spell ngSuggest onCand date( d, textResult, spell ngAct onType, or g nalQuery)
}

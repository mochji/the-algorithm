...
tra  BaseDMConvoCand date extends Un versalNoun[Str ng] {
  def lastReadableEvent d: Opt on[Long]
}

/**
 * Canon cal DMConvoCand date model. Always prefer t  vers on over all ot r var ants.
 *
 ...
 */
f nal class DMConvoCand date pr vate (
  overr de val  d: Str ng,
  overr de val lastReadableEvent d: Opt on[Long])
    extends BaseDMConvoCand date {

  /**
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[DMConvoCand date]
        ...
        (
          (t  eq cand date)
            || ((hashCode == cand date.hashCode)
              && ( d == cand date. d && lastReadableEvent d == cand date.lastReadableEvent d))
        )
      case _ =>
        false
    }
...
object DMConvoCand date {
  def apply( d: Str ng, lastReadableEvent d: Opt on[Long]): DMConvoCand date =
    new DMConvoCand date( d, lastReadableEvent d)
}

/**
 * Canon cal DMConvoSearchCand date model. Always prefer t  vers on over all ot r var ants.
 *
 * @note Any add  onal f elds should be added as a [[com.tw ter.product_m xer.core.feature.Feature]]
 *       on t  cand date's [[com.tw ter.product_m xer.core.feature.featuremap.FeatureMap]].  f t 
 *       features co  from t  cand date s ce  self (as opposed to hydrated v a a
 ...
 *       (see note on t  equals  thod below)
 */
f nal class DMConvoSearchCand date pr vate (
  overr de val  d: Str ng,
  overr de val lastReadableEvent d: Opt on[Long])
    extends BaseDMConvoCand date {
   ...
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[DMConvoSearchCand date]
...
object DMConvoSearchCand date {
  def apply( d: Str ng, lastReadableEvent d: Opt on[Long]): DMConvoSearchCand date =
    new DMConvoSearchCand date( d, lastReadableEvent d)
}

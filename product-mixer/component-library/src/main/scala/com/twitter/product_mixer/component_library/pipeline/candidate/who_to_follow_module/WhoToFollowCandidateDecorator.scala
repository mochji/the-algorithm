package com.tw ter.product_m xer.component_l brary.p pel ne.cand date.who_to_follow_module
 ...
 mport com.tw ter.product_m xer.component_l brary.cand date_s ce.people_d scovery.WhoToFollowModule aderFeature
 mport com.tw ter.product_m xer.component_l brary.cand date_s ce.people_d scovery.WhoToFollowModuleShowMoreFeature
 ...
 mport com.tw ter.product_m xer.component_l brary.decorator.urt.bu lder.t  l ne_module.ModuleDynam cShowMoreBehav orRevealByCountBu lder
...
object WhoToFollowCand dateDecorator {
  val Cl entEventComponent = "suggest_who_to_follow"
  val EntryNa spaceStr ng = "who-to-follow"
}

case class WhoToFollowCand dateDecorator[-Query <: P pel neQuery](
  ...
  overr de def apply(
    ...
    val cl entEvent nfoBu lder = Cl entEvent nfoBu lder[Query, UserCand date](
      WhoToFollowCand dateDecorator.Cl entEventComponent,
    ...
    val whoToFollowModuleBu lder = {
      ...
      val whoToFollowModule aderBu lder = whoToFollow aderOpt.flatMap(_.t le).map { t le =>
        Module aderBu lder(textBu lder = StrStat c(t le.text),  sSt cky = So (true))
      }
      val whoToFollowModuleFooterBu lder = whoToFollow aderOpt.flatMap(_.act on).map { act on =>
        ModuleFooterBu lder(
          textBu lder = StrStat c(act on.t le),
          urlBu lder = So (Stat cUrlBu lder(act on.act onUrl, DeepL nk)))
      }
      val showMoreBehav orBu lder =
        query.features.flatMap(_.get(WhoToFollowModuleShowMoreFeature)).map { showMore =>
          ModuleDynam cShowMoreBehav orRevealByCountBu lder(
            showMore. n  alToShow,
            showMore.extraToShow)
        }

      T  l neModuleBu lder(
        entryNa space = EntryNa space(WhoToFollowCand dateDecorator.EntryNa spaceStr ng),
        cl entEvent nfoBu lder = cl entEvent nfoBu lder,
        ...
        showMoreBehav orBu lder = showMoreBehav orBu lder
      )
    }

...
object F natraHttpCl entW hCredent alProxyModule extends Tw terModule {

  f nal val F natraHttpCl entW hCredent alProxy = "F nagleHttpCl entW hCredent alProxy"

  /**
   * Bu ld a F natra HTTP cl ent w h Egress Proxy support w h Credent als for a host. T  F natra
   * HTTP cl ent can be  lpful (as opposed to t  base F nagle HTTP Cl ent), as   prov des
   * bu lt- n JSON response zpars ng and ot r conven ence  thods
   *
  ...
  @Prov des
  @S ngleton
  @Na d(F natraHttpCl entW hCredent alProxy)
  def prov desF natraHttpCl entW hCredent alProxy(
    ...
    val f nagleHttpCl entW hCredent alProxy =
    ...
    val f nagleHttpServ ceW hCredent alProxy =
      bu ldF nagleHttpServ ceW hProxy(
        f nagleHttpCl entW hProxy = f nagleHttpCl entW hCredent alProxy,
        tw terProxyHostPort = tw terProxyHostPort
      )

    new HttpCl ent(
      hostna  = tw terProxyHostPort.host,
      httpServ ce = f nagleHttpServ ceW hCredent alProxy,

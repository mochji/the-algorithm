...
/**
 * A [[P pel neResultS deEffect]] that logs [[Thr ft]] data may not already be ava lable to Scr be
 */
tra  Scr beLogEventAsyncS deEffect[
  Thr ft <: Thr ftStruct,
  Query <: P pel neQuery,
  ...
  f nal overr de def apply(
    ...
    logEvents.flatMap { logEvents: Seq[Thr ft] =>
      St ch.collect {

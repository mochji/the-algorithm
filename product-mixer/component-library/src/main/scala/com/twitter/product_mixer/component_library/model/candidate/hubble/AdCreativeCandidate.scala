...
/**
 * Canon cal AdCreat veCand date model wh ch descr bes an Ad Creat ve from an ad manage nt
 * perspect ve.   can be a t et, or account, and has a 1:n relat onsh p w h ad un s. Always
 * prefer t  vers on over all ot r var ants.
 *
 ...
 */
f nal class AdCreat veCand date pr vate (
  // T   s t  creat ve d, but needs to be na d  D to conf rm to Un versalNoun
  overr de val  d: Long,
  val adType: AdType,
   ...
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[AdCreat veCand date]
        ...
        (
          (t  eq cand date)
            || ((hashCode == cand date.hashCode)
              && ( d == cand date. d && adType == cand date.adType && adAccount d == cand date.adAccount d))
        )
      case _ =>
        false
    }
...
object AdCreat veCand date {
  def apply(
     d: Long,
    adType: AdType,
    adAccount d: Long
  ): AdCreat veCand date =
    new AdCreat veCand date( d, adType, adAccount d)
}

...
/**
 * Canon cal Tw terL stCand date model. Always prefer t  vers on over all ot r var ants.
 *
 ...
 */
f nal class Tw terL stCand date pr vate (
  overr de val  d: Long)
   ...
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[Tw terL stCand date]
...
object Tw terL stCand date {
  def apply( d: Long): Tw terL stCand date = new Tw terL stCand date( d)
}

...
/**
 * Canon cal AdUn Cand date model wh ch descr bes an "Ad" from t  Ad Manage nt perspect ve.    s
 * based on t  AdUn  table  n Ads DB, and prov des a cand date for advert sers to manage and
 * report on t  r advert s ng conf gurat ons.Always prefer t  vers on over all ot r var ants.
 *
 ...
 */
f nal class AdUn Cand date pr vate (
  // T   s t  adUn  d, but needs to be na d  D to conf rm to Un versalNoun
  overr de val  d: Long,
   ...
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[AdUn Cand date]
...
object AdUn Cand date {
  def apply(
     d: Long,
    adAccount d: Long
  ): AdUn Cand date =
    new AdUn Cand date( d, adAccount d)
}

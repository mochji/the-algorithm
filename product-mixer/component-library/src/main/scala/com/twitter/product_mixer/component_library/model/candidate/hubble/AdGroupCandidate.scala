...
/**
 * Canon cal AdGroupCand date model wh ch descr bes an "Ad Group" from t  t  Ad Manage nt
 * perspect ve.    s based on t  L ne em table  n Ads DB, and prov des an ad group for
 * advert sers to manage and report d fferent l ne  ems belong ng to a s ngle ad. Always prefer
 * t  vers on over all ot r var ants.
 *
 ...
 */
f nal class AdGroupCand date pr vate (
  overr de val  d: Long, // T   s t  ad_group_ d, rena d to  D to conform to Un versalNoun
   ...
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[AdGroupCand date]
        ...
        (
          (t  eq cand date)
            || ((hashCode == cand date.hashCode)
              && ( d == cand date. d && adAccount d == cand date.adAccount d))
        )
      case _ =>
        false
    }
...
object AdGroupCand date {
  def apply(
     d: Long,
    adAccount d: Long
  ): AdGroupCand date =
    new AdGroupCand date( d, adAccount d)
}

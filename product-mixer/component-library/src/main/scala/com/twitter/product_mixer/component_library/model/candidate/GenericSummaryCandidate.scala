...
/**
 * Canon cal Gener cSummaryCand date model. Always prefer t  vers on over all ot r var ants.
 *
 ...
 */
f nal class Gener cSummaryCand date pr vate (
   ...
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[Gener cSummaryCand date]
  ...
  overr de def equals(that: Any): Boolean =
    that match {
      case cand date: Gener cSummaryCand date =>
        (
          (t  eq cand date)
...
object Gener cSummaryCand date {
  def apply( d: Str ng): Gener cSummaryCand date = new Gener cSummaryCand date( d)
}

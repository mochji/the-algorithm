...
/**
 * Canon cal Fund ngS ceCand date model wh ch descr bes a "Fund ng  nstru nt" from t  t  Ad
 * Manage nt perspect ve. Always prefer t  vers on over all ot r var ants.
 *
 ...
 */
f nal class Fund ngS ceCand date pr vate (
  overr de val  d: Long,
   ...
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[Fund ngS ceCand date]
...
object Fund ngS ceCand date {
  def apply(
     d: Long,
    adAccount d: Long
  ): Fund ngS ceCand date = new Fund ngS ceCand date( d, adAccount d)
}

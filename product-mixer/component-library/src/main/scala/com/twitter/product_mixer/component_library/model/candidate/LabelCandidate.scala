...
/**
 * Canon cal LabelCand date model. Always prefer t  vers on over all ot r var ants.
 *
 ...
 */
f nal class LabelCand date pr vate (
  overr de val  d: Long)
   ...
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[LabelCand date]
...
object LabelCand date {
  def apply( d: Long): LabelCand date = new LabelCand date( d)
}

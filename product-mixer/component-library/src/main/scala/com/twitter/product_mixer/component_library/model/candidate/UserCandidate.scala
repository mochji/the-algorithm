...
// do not need t m.
@JsonTypeNa ("user")
tra  BaseUserCand date extends Un versalNoun[Long]

/**
 * Canon cal UserCand date model. Always prefer t  vers on over all ot r var ants.
 *
 ...
 */
f nal class UserCand date pr vate (
  overr de val  d: Long)
    extends BaseUserCand date {

  /**
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[UserCand date]
...
object UserCand date {
  def apply( d: Long): UserCand date = new UserCand date( d)
}

/**
 * Feature to  nd cate w t r a rendered user cand date should be marked unread  n URT. Used  n
 * [[UserCand dateUrt emBu lder]] w n decorat ng t  cand date.r
 */
object  sMarkUnreadFeature extends Feature[BaseUserCand date, Boolean]

...
/**
 * Keep only t  f rst  nstance of a cand date  n t  `result` as determ ned by compar ng
 ...
 * t n t  output result w ll be `Seq(s ceA_ d1, s ceA_ d2, s ceB_ d3, s ceC_ d4)`
 */
case class DropDupl cateResults(
  ...
  overr de def apply(
    ...
    val dedupedResults = dropDupl cates(
      p pel neScope = p pel neScope,
      cand dates = result,
      dupl cat onKey = dupl cat onKey,
       rgeStrategy =  rgeStrategy)

    SelectorResult(rema n ngCand dates = rema n ngCand dates, result = dedupedResults)
  }
}

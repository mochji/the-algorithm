...
tra  BaseArt cleCand date extends Un versalNoun[ nt]

/**
 * Canon cal Art cleCand date model. Always prefer t  vers on over all ot r var ants.
 *
 ...
 */
f nal class Art cleCand date pr vate (
  overr de val  d:  nt)
    extends BaseArt cleCand date {

  /**
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[Art cleCand date]
...
object Art cleCand date {
  def apply( d:  nt): Art cleCand date = new Art cleCand date( d)
}

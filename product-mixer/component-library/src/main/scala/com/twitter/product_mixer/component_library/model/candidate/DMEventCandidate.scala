...
/**
 * Canon cal DM Events such as  ssage Create, Conversat on Create, Jo n conversat on, etc model.
 * Always prefer t  vers on over all ot r var ants.
 *
 ...
 */
f nal class DMEventCand date pr vate (
  overr de val  d: Long)
   ...
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[DMEventCand date]
...
object DMEventCand date {
  def apply( d: Long): DMEventCand date = new DMEventCand date( d)
}

/**
 * Represent DM Events such as  ssage Create, Conversat on Create, Jo n conversat on, etc.
 *
 * @note  tor cally t  was used to represent events from Elast c
 *       Search rat r than Strato. Now deprecated  n favor of DMEvent.
 *
 * @note Any add  onal f elds should be added as a [[com.tw ter.product_m xer.core.feature.Feature]]
 *       on t  cand date's [[com.tw ter.product_m xer.core.feature.featuremap.FeatureMap]].  f t 
 *       features co  from t  cand date s ce  self (as opposed to hydrated v a a
...
f nal class DM ssageSearchCand date pr vate (
  overr de val  d: Long)
    extends Un versalNoun[Long] {
   ...
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[DM ssageSearchCand date]
  ...
  overr de def equals(that: Any): Boolean =
    that match {
      case cand date: DM ssageSearchCand date =>
        (
          (t  eq cand date)
            || ((hashCode == cand date.hashCode)
              && ( d == cand date. d))
...
object DM ssageSearchCand date {
  def apply( d: Long): DM ssageSearchCand date = new DM ssageSearchCand date( d)
}

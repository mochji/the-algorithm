...
/**
 * Canon cal CursorCand date model. Always prefer t  vers on over all ot r var ants.
 *
 ...
 */
f nal class CursorCand date pr vate (
  overr de val  d: Long,
  val value: Str ng,
   ...
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[CursorCand date]
        ...
        (
          (t  eq cand date)
            || ((hashCode == cand date.hashCode)
              && ( d == cand date. d && value == cand date.value && cursorType == cand date.cursorType))
        )
      case _ =>
        false
    }
...
object CursorCand date {
  def apply( d: Long, value: Str ng, cursorType: CursorType): CursorCand date =
    new CursorCand date( d, value, cursorType)
}

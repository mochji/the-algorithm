...
/**
 * Canon cal Mo ntAnnotat onCand date model. Always prefer t  vers on over all ot r var ants.
 *
 ...
 */
f nal class Mo ntAnnotat onCand date pr vate (
  overr de val  d: Long,
  val text: Opt on[Str ng],
  val  ader: Opt on[Str ng])
    extends Un versalNoun[Long] {

  /**
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[Mo ntAnnotat onCand date]
  ...
  overr de def equals(that: Any): Boolean =
    that match {
      case cand date: Mo ntAnnotat onCand date =>
        (
          (t  eq cand date)
            || ((hashCode == cand date.hashCode)
              && ( d == cand date. d && text == cand date.text &&  ader == cand date. ader))
        )
      case _ =>
        false
    }
...
object Mo ntAnnotat onCand date {
  def apply(
     d: Long,
    text: Opt on[Str ng],
     ader: Opt on[Str ng]
  ): Mo ntAnnotat onCand date = new Mo ntAnnotat onCand date( d, text,  ader)
}

...
/**
 * Canon cal CardCand date model. Always prefer t  vers on over all ot r var ants.
 *
 ...
 */
f nal class CardCand date pr vate (
   ...
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[CardCand date]
...
object CardCand date {
  def apply( d: Str ng): CardCand date = new CardCand date( d)
}

 ...
 */
sealed abstract class ProductP pel ne dent f er(overr de val na : Str ng)
    extends Component dent f er("ProductP pel ne", na ) {

  /**
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[ProductP pel ne dent f er]
  ...
  overr de def equals(that: Any): Boolean =
    that match {
      case  dent f er: ProductP pel ne dent f er =>
...
object ProductP pel ne dent f er {
  def apply(na : Str ng)( mpl c  s ceF le: s cecode.F le): ProductP pel ne dent f er = {
     f (Component dent f er. sVal dNa (na ))
      new ProductP pel ne dent f er(na ) {
        overr de val f le: s cecode.F le = s ceF le
      }
    else
      throw new  llegalArgu ntExcept on(s" llegal ProductP pel ne dent f er: $na ")
  }
}

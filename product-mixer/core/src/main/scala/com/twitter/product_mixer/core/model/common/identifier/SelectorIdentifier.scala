 ...
 */
sealed abstract class Selector dent f er(overr de val na : Str ng)
    extends Component dent f er("Selector", na ) {

  /**
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[Selector dent f er]
  ...
  overr de def equals(that: Any): Boolean =
    that match {
      case  dent f er: Selector dent f er =>
...
/**
 * Used  n `SelectorExecutor` to g ve an  D to each selector  n t  `Component dent f erStack`
 *
 * T se are often generated automat cally  n t  executor and are dependent on t  class na s
 * so   sk p val dat on to avo d  ssues. S nce   dont record stats for Selectors t   s okay.
 */
pr vate[core] object Selector dent f er {
  def apply(
    na : Str ng,
     ndex:  nt
  )(
     mpl c  s ceF le: s cecode.F le
  ): Selector dent f er = {
    val cap al zedW houtSpec alCharacters = na .replace("$", "").cap al ze
    new Selector dent f er( ndex.toStr ng + cap al zedW houtSpec alCharacters) {

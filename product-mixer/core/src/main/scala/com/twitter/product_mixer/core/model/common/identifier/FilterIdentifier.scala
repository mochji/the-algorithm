 ...
 */
sealed abstract class F lter dent f er(overr de val na : Str ng)
    extends Component dent f er("F lter", na ) {

  /**
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[F lter dent f er]
  ...
  overr de def equals(that: Any): Boolean =
    that match {
      case  dent f er: F lter dent f er =>
...
object F lter dent f er {
  def apply(na : Str ng)( mpl c  s ceF le: s cecode.F le): F lter dent f er = {
     f (Component dent f er. sVal dNa (na ))
      new F lter dent f er(na ) {
        overr de val f le: s cecode.F le = s ceF le
      }
    else
      throw new  llegalArgu ntExcept on(s" llegal F lter dent f er: $na ")
  }
}

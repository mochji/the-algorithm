...
/**
 * [[Component dent f er]] type used by  nternal parts of Product M xer that need to be  dent f ed
 *
 ...
 */
sealed abstract class Platform dent f er(overr de val na : Str ng)
    extends Component dent f er("Platform", na ) {

  /**
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[Platform dent f er]
  ...
  overr de def equals(that: Any): Boolean =
    that match {
      case  dent f er: Platform dent f er =>
...
object Platform dent f er {
  def apply(na : Str ng)( mpl c  s ceF le: s cecode.F le): Platform dent f er = {
     f (Component dent f er. sVal dNa (na ))
      new Platform dent f er(na ) {
        overr de val f le: s cecode.F le = s ceF le
      }
    else
      throw new  llegalArgu ntExcept on(s" llegal Platform dent f er: $na ")
  }
}

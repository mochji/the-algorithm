 ...
 */
sealed abstract class Gate dent f er(overr de val na : Str ng)
    extends Component dent f er("Gate", na ) {

  /**
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[Gate dent f er]
...
object Gate dent f er {
  def apply(na : Str ng)( mpl c  s ceF le: s cecode.F le): Gate dent f er = {
     f (Component dent f er. sVal dNa (na ))
      new Gate dent f er(na ) {
        overr de val f le: s cecode.F le = s ceF le
      }
    else
      throw new  llegalArgu ntExcept on(s" llegal Gate dent f er: $na ")
  }
}

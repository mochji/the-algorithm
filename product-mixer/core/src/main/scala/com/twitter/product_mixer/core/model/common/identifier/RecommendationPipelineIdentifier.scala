...
/**
 * Recom ndat on P pel ne  dent f er
 *
 ...
 */
sealed abstract class Recom ndat onP pel ne dent f er(overr de val na : Str ng)
    extends Component dent f er("Recom ndat onP pel ne", na ) {

  /**
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[Recom ndat onP pel ne dent f er]
  ...
  overr de def equals(that: Any): Boolean =
    that match {
      case  dent f er: Recom ndat onP pel ne dent f er =>
...
object Recom ndat onP pel ne dent f er {
  def apply(
    na : Str ng
  )(
     mpl c  s ceF le: s cecode.F le
  ): Recom ndat onP pel ne dent f er = {
     f (Component dent f er. sVal dNa (na ))
      new Recom ndat onP pel ne dent f er(na ) {
        overr de val f le: s cecode.F le = s ceF le
      }
    else
      throw new  llegalArgu ntExcept on(s" llegal Recom ndat onP pel ne dent f er: $na ")
  }
}

 ...
 */
sealed abstract class Cand dateS ce dent f er(overr de val na : Str ng)
    extends Component dent f er("Cand dateS ce", na ) {

  /**
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[Cand dateS ce dent f er]
  ...
  overr de def equals(that: Any): Boolean =
    that match {
      case  dent f er: Cand dateS ce dent f er =>
...
object Cand dateS ce dent f er {
  def apply(na : Str ng)( mpl c  s ceF le: s cecode.F le): Cand dateS ce dent f er = {
     f (Component dent f er. sVal dNa (na ))
      new Cand dateS ce dent f er(na ) {
        overr de val f le: s cecode.F le = s ceF le
      }
    else
      throw new  llegalArgu ntExcept on(s" llegal Cand dateS ce dent f er: $na ")
  }
}

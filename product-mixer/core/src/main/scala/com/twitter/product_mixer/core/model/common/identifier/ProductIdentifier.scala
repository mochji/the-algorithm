 ...
 */
sealed abstract class Product dent f er(overr de val na : Str ng)
    extends Component dent f er("Product", na ) {

  /**
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[Product dent f er]
  ...
  overr de def equals(that: Any): Boolean =
    that match {
      case  dent f er: Product dent f er =>
...
object Product dent f er {
  def apply(na : Str ng)( mpl c  s ceF le: s cecode.F le): Product dent f er = {
     f (Component dent f er. sVal dNa (na ))
      new Product dent f er(na ) {
        overr de val f le: s cecode.F le = s ceF le
      }
    else
      throw new  llegalArgu ntExcept on(s" llegal Product dent f er: $na ")
  }
}

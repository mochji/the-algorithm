 ...
 */
sealed abstract class Scorer dent f er(overr de val na : Str ng)
    extends Component dent f er("Scorer", na ) {

  /**
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[Scorer dent f er]
  ...
  overr de def equals(that: Any): Boolean =
    that match {
      case  dent f er: Scorer dent f er =>
...
object Scorer dent f er {
  def apply(na : Str ng)( mpl c  s ceF le: s cecode.F le): Scorer dent f er = {
     f (Component dent f er. sVal dNa (na ))
      new Scorer dent f er(na ) {
        overr de val f le: s cecode.F le = s ceF le
      }
    else
      throw new  llegalArgu ntExcept on(s" llegal Scorer dent f er: $na ")
  }
}

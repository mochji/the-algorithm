...
/**
 * Transport Marshaller  dent f er
 *
 ...
 */
sealed abstract class TransportMarshaller dent f er(overr de val na : Str ng)
    extends Component dent f er("TransportMarshaller", na ) {

  /**
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[TransportMarshaller dent f er]
  ...
  overr de def equals(that: Any): Boolean =
    that match {
      case  dent f er: TransportMarshaller dent f er =>
...
object TransportMarshaller dent f er {
  def apply(na : Str ng)( mpl c  s ceF le: s cecode.F le): TransportMarshaller dent f er = {
     f (Component dent f er. sVal dNa (na ))
      new TransportMarshaller dent f er(na ) {
        overr de val f le: s cecode.F le = s ceF le
      }
    else
      throw new  llegalArgu ntExcept on(s" llegal TransportMarshaller dent f er: $na ")
  }
}

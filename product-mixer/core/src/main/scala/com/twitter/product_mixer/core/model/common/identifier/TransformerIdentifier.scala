 ...
 */
sealed abstract class Transfor r dent f er(overr de val na : Str ng)
    extends Component dent f er("Transfor r", na ) {

  /**
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[Transfor r dent f er]
  ...
  overr de def equals(that: Any): Boolean =
    that match {
      case  dent f er: Transfor r dent f er =>
...
object Transfor r dent f er {
  def apply(na : Str ng)( mpl c  s ceF le: s cecode.F le): Transfor r dent f er = {
     f (Component dent f er. sVal dNa (na ))
      new Transfor r dent f er(na ) {
        overr de val f le: s cecode.F le = s ceF le
      }
    else
      throw new  llegalArgu ntExcept on(s" llegal Transfor r dent f er: $na ")
  }
}

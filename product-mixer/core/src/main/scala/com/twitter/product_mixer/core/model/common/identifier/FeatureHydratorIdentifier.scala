 ...
 */
sealed abstract class FeatureHydrator dent f er(overr de val na : Str ng)
    extends Component dent f er("FeatureHydrator", na ) {

  /**
   * @ n r doc
   */
  overr de def canEqual(that: Any): Boolean = that. s nstanceOf[FeatureHydrator dent f er]
  ...
  overr de def equals(that: Any): Boolean =
    that match {
      case  dent f er: FeatureHydrator dent f er =>
...
object FeatureHydrator dent f er {
  def apply(na : Str ng)( mpl c  s ceF le: s cecode.F le): FeatureHydrator dent f er = {
     f (Component dent f er. sVal dNa (na ))
      new FeatureHydrator dent f er(na ) {
        overr de val f le: s cecode.F le = s ceF le
      }
    else
      throw new  llegalArgu ntExcept on(s" llegal FeatureHydrator dent f er: $na ")
  }
}

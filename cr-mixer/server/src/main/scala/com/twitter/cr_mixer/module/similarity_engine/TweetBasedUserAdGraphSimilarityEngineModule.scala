...
object T etBasedUserAdGraphS m lar yEng neModule extends Tw terModule {
  ...
  def prov desT etBasedUserAdGraphS m lar yEng ne(
    ...
    val underly ngStore = T etBasedUserAdGraphS m lar yEng ne(
      userAdGraphServ ce,
    ...
    val  mCac dStore: ReadableStore[
      ...
      Observed mcac dReadableStore
        .fromCac Cl ent(
          ...
          statsRece ver = statsRece ver.scope("t et_based_user_ad_graph_store_ mcac "),
          keyToStr ng = { k =>
            //Example Query CRM xer:T etBasedUTG:1234567890ABCDEF
            f"CRM xer:T etBasedUAG:${keyHas r.hashKey(k.toStr ng.getBytes)}%X"
          }
        )
    ...
    new StandardS m lar yEng ne[
      ...
      T etW hScore
       ...
       dent f er = S m lar yEng neType.T etBasedUserAdGraph,

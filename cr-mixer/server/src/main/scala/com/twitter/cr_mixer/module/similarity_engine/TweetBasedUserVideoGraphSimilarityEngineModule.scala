...
object T etBasedUserV deoGraphS m lar yEng neModule extends Tw terModule {
  ...
  def prov desT etBasedUserV deoGraphS m lar yEng ne(
    ...
    val  mCac dStore: ReadableStore[
      ...
      Observed mcac dReadableStore
        .fromCac Cl ent(
          ...
          statsRece ver = statsRece ver.scope("t et_based_user_v deo_graph_store_ mcac "),
          keyToStr ng = { k =>
            //Example Query CRM xer:T etBasedUVG:1234567890ABCDEF
            f"CRM xer:T etBasedUVG:${keyHas r.hashKey(k.toStr ng.getBytes)}%X"
          }
        )
    ...
    new StandardS m lar yEng ne[
      ...
      T etW hScore
       ...
       dent f er = S m lar yEng neType.T etBasedUserV deoGraph,

      ...
      _.map { s gnals =>
        ...
        lookBackW ndowF lteredS gnals
          .f lter { recentEngagedT et =>
            ...
            applyS gnalT etTypeF lter(query.s gnalType, recentEngagedT et)
          }.collect {
            case RecentEngagedT et(t et d, engagedAt, _: Favor e tadata, _) =>
              S gnal(query.s gnalType, engagedAt, So ( nternal d.T et d(t et d)))
          }.take(query.maxResults.getOrElse( nt.MaxValue))
      }
    }
  }
  pr vate def applyS gnalT etTypeF lter(
    s gnal: S gnalType,
    recentEngagedT et: RecentEngagedT et
  ): Boolean = {
    // Perform spec f c f lters for part cular s gnal types.
    s gnal match {
      case S gnalType.AdFavor e => S gnalF lter. sPromotedT et(recentEngagedT et)
      case _ => true
    }
  }
}

object T etFavor esFetc r {

...
class FeatureStoreG zmoduckS ce @ nject() (
  serv ce dent f er: Serv ce dent f er,
  stats: StatsRece ver)
    extends FeatureS ce {
   mport FeatureStoreG zmoduckS ce._

  val backupS ceStats = stats.scope("feature_store_hydrat on_g zmoduck")
  val adapterStats = backupS ceStats.scope("adapters")
  overr de def  d: FeatureS ce d = FeatureS ce d.FeatureStoreG zmoduckS ce d
...
object FeatureStoreG zmoduckS ce {
  pr vate val DatasetCac Scope = "feature_store_local_cac _g zmoduck"
  pr vate val DefaultCac MaxKeys = 20000

  val allFeatures: Set[BoundFeature[_ <: Ent y d, _]] =
    FeatureStoreFeatures.cand dateUserStatusFeatures ++
      FeatureStoreFeatures.s m larToUserStatusFeatures ++
    ...
    Dynam cHydrat onConf g(
      Set(
        GatedFeatures(
          ...
          gate = HasParams
            .paramGate(FeatureStoreS ceParams.EnableSeparateCl entForG zmoduck) &
            HasParams.paramGate(FeatureStoreS ceParams.EnableTargetUserFeatures)
        ),
        GatedFeatures(
          boundFeatureSet = BoundFeatureSet(FeatureStoreFeatures.cand dateUserStatusFeatures),
          gate =
            HasParams
              .paramGate(FeatureStoreS ceParams.EnableSeparateCl entForG zmoduck) &
        ...
        GatedFeatures(
          boundFeatureSet = BoundFeatureSet(FeatureStoreFeatures.s m larToUserStatusFeatures),
          gate =
            HasParams
              .paramGate(FeatureStoreS ceParams.EnableSeparateCl entForG zmoduck) &

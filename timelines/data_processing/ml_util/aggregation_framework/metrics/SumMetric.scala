...
case class TypedSum tr c(
) extends TypedSumL ke tr c[JDouble] {
   mport Aggregat on tr cCommon._

  overr de val operatorNa  = "sum"

  /*
   * Transform feature ->  s value  n t  g ven record,
   * or 0 w n feature = None (sum has no  an ng  n t  case)
   */
  overr de def get ncre ntValue(
    ...
    case So (f) => {
      T  dValue[Double](
        value = Opt on(SR chDataRecord(record).getFeatureValue(f)).map(_.toDouble).getOrElse(0.0),
...
/**
 * Syntact c sugar for t  sum  tr c that works w h cont nuous features.
 * See Easy tr c.scala for more deta ls on why t   s useful.
 */
object Sum tr c extends Easy tr c {
  overr de def forFeatureType[T](
    featureType: FeatureType
  ): Opt on[Aggregat on tr c[T, _]] =
    featureType match {
      case FeatureType.CONT NUOUS =>
        So (TypedSum tr c().as nstanceOf[Aggregat on tr c[T, Double]])
      case _ => None
    }
}

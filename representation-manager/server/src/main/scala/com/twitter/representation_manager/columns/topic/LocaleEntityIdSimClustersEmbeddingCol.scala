...
class LocaleEnt y dS mClustersEmbedd ngCol @ nject() (
  embedd ngStore: Top cS mClustersEmbedd ngStore)
    extends StratoFed.Column(
      "recom ndat ons/representat on_manager/s mClustersEmbedd ng.LocaleEnt y d")
  ...
  overr de type Key = LocaleEnt y d
  overr de type V ew = S mClustersEmbedd ngV ew
  overr de type Value = S mClustersEmbedd ng

  overr de val keyConv: Conv[Key] = ScroogeConv.fromStruct[LocaleEnt y d]
  ...
  overr de val  tadata: Op tadata = Op tadata(
    ...
    descr pt on = So (
      Pla nText(
        "T  Top c S mClusters Embedd ng Endpo nt  n Representat on Manage nt Serv ce w h LocaleEnt y d." +
          " TDD: http://go/rms-tdd"))
  )

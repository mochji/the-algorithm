...
class BatchPred ct onTensorWr er(Layer):
  """
  A layer that packages keys and dense tensors  nto a BatchPred ct onResponse.
  Typ cally used at t  out of an exported model for use  n a t  Pred ct onEng ne
  (that  s,  n product on) w n model pred ct ons are dense tensors.
  ...
  def __ n __(self, keys, **kwargs):  # pyl nt: d sable=useless-super-delegat on
    super(BatchPred ct onTensorWr er, self).__ n __(**kwargs)
    self.keys = keys
  ...
  def call(self, values, **kwargs):  # pyl nt: d sable=unused-argu nt, argu nts-d ffer
    ...
    """
    wr e_op = l btwml.ops.batch_pred ct on_tensor_response_wr er(self.keys, values)
    return wr e_op

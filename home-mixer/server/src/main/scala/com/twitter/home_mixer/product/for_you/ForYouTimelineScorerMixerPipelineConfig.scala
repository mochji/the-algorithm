...
@S ngleton
class For T  l neScorerM xerP pel neConf g @ nject() (
  ...
  g zmoduckUserQueryFeatureHydrator: G zmoduckUserQueryFeatureHydrator,
   mpress onBloomF lterQueryFeatureHydrator:  mpress onBloomF lterQueryFeatureHydrator[For Query],
  manhattanT et mpress onsQueryFeatureHydrator: T et mpress onsQueryFeatureHydrator[For Query],
  ...
  updateLastNonPoll ngT  S deEffect: UpdateLastNonPoll ngT  S deEffect[For Query, T  l ne],
  ...
  overr de val  dent f er: M xerP pel ne dent f er = M xerP pel ne dent f er("For T  l neScorer")
  ...
  overr de def fetchQueryFeatures: Seq[QueryFeatureHydrator[For Query]] = Seq(
    requestQueryFeatureHydrator,
    pers stenceStoreQueryFeatureHydrator,
    t  l neServ ceT etsQueryFeatureHydrator,
    feedback toryQueryFeatureHydrator,
  ...
  pr vate val t  l neScorerCand dateP pel neScope =
    Spec f cP pel ne(for T  l neScorerCand dateP pel neConf g. dent f er)

  pr vate val for AdsCand dateP pel neConf g = for AdsDependentCand dateP pel neBu lder
    .bu ld(t  l neScorerCand dateP pel neScope)
  ...
  overr de val cand dateP pel nes: Seq[Cand dateP pel neConf g[For Query, _, _, _]] = Seq(
    for T  l neScorerCand dateP pel neConf g,
  ...
  overr de val fa lOpenPol c es: Map[Cand dateP pel ne dent f er, Fa lOpenPol cy] = Map(
    for T  l neScorerCand dateP pel neConf g. dent f er -> Fa lOpenPol cy.Always,
  ...
  overr de val resultSelectors: Seq[Selector[For Query]] = Seq(
    ...
    UpdateSortCand dates(
      ...
      cand dateP pel ne = for T  l neScorerCand dateP pel neConf g. dent f er
    ),
    ...
    DebunchCand dates(
      p pel neScope = Spec f cP pel ne(for T  l neScorerCand dateP pel neConf g. dent f er),
      mustDebunch = {
    ...
    // Add Conversat on Serv ce t ets to results only  f t  scored p pel ne doesn't return any
    SelectCond  onally(
      selector =  nsertAppendResults(
        cand dateP pel ne = for Conversat onServ ceCand dateP pel neConf g. dent f er),
       ncludeSelector = (_, cand dates, _) =>
        !cand dates.ex sts(cand date =>
          for T  l neScorerCand dateP pel neConf g. dent f er == cand date.s ce)
    ),
     nsertAppendResults(cand dateP pel ne = for T  l neScorerCand dateP pel neConf g. dent f er),
  ...
  pr vate val servedCand dateKeysKafkaS deEffect =
    servedCand dateKeysKafkaS deEffectBu lder.bu ld(
      Set(for T  l neScorerCand dateP pel neConf g. dent f er))
  ...
  pr vate val ho Scr beCl entEventS deEffect = Ho Scr beCl entEventS deEffect(
    ...
    whoToFollowCand dateP pel ne dent f er =
      So (for WhoToFollowCand dateP pel neConf g. dent f er),

package com.tw ter.ho _m xer.ut l.t etyp e.content
 ...
 mport com.tw ter. d aserv ces.commons. d a nformat on.{thr ftscala => m }
 mport com.tw ter. d aserv ces.commons.t et d a.{thr ftscala => tm}
 mport com.tw ter. d aserv ces.commons.{thr ftscala => ms}
 mport com.tw ter.t etyp e.{thr ftscala => tp}
 mport scala.collect on.Map

object T et d aFeaturesExtractor {

  pr vate val  mageCategor es = Set(
    ms. d aCategory.T et mage.value,
    ms. d aCategory.T etG f.value
  )
  pr vate val V deoCategor es = Set(
    ms. d aCategory.T etV deo.value,
    ms. d aCategory.Ampl fyV deo.value
  )

  def has mage(t et: tp.T et): Boolean = has d aByCategory(t et,  mageCategor es)

  def hasV deo(t et: tp.T et): Boolean = has d aByCategory(t et, V deoCategor es)

  pr vate def has d aByCategory(t et: tp.T et, categor es: Set[ nt]): Boolean = {
    t et. d a.ex sts {  d aEnt  es =>
       d aEnt  es.ex sts {  d aEnt y =>
         d aEnt y. d aKey.map(_. d aCategory).ex sts {  d aCategory =>
          categor es.conta ns( d aCategory.value)
        }
      }
    }
  }
  ...
  pr vate def getS zeFeatures( d aEnt  es: Seq[tp. d aEnt y]): Seq[ d aS zeFeatures] = {
  ...
  pr vate def getPlaybackFeatures( d aEnt  es: Seq[tp. d aEnt y]): PlaybackFeatures = {
    val allPlaybackFeatures =  d aEnt  es
      .flatMap {  d aEnt y =>
         d aEnt y. d a nfo map {
          case v deoEnt y: tm. d a nfo.V deo nfo =>
            PlaybackFeatures(
          ...
          case g fEnt y: tm. d a nfo.An matedG f nfo =>
            PlaybackFeatures(
              durat onMs = None,
  ...
  pr vate def get d aTagScreenNa s(tagMap: Map[Long, Seq[tp. d aTag]]): Seq[Str ng] =
    tagMap.values
      .flatMap(seq d aTag => seq d aTag.flatMap(_.screenNa ))
      .toSeq

  // Areas of t  faces  dent f ed  n t   d a ent  es
  pr vate def getFaceMapAreas( d aEnt  es: Seq[tp. d aEnt y]): Seq[ nt] = {
    for {
       d aEnt y <-  d aEnt  es
  ...
  pr vate def getSt ckerFeatures( d aEnt  es: Seq[tp. d aEnt y]): Seq[Long] = {
    for {
       d aEnt y <-  d aEnt  es
       tadata <-  d aEnt y.add  onal tadata.toSeq
  ...
  pr vate def get d aOr g nProv ders( d aEnt  es: Seq[tp. d aEnt y]): Seq[Str ng] =
    for {
       d aEnt y <-  d aEnt  es
       tadata <-  d aEnt y.add  onal tadata.toSeq
  ...
  pr vate def get sManaged( d aEnt  es: Seq[tp. d aEnt y]): Boolean = {
    for {
       d aEnt y <-  d aEnt  es
       tadata <-  d aEnt y.add  onal tadata.toSeq
  ...
  pr vate def get s360( d aEnt  es: Seq[tp. d aEnt y]): Boolean = {
    for {
       d aEnt y <-  d aEnt  es
       tadata <-  d aEnt y.add  onal tadata.toSeq
  ...
  pr vate def getV ewCount( d aEnt  es: Seq[tp. d aEnt y]): Opt on[Long] = {
    for {
       d aEnt y <-  d aEnt  es
       tadata <-  d aEnt y.add  onal tadata.toSeq

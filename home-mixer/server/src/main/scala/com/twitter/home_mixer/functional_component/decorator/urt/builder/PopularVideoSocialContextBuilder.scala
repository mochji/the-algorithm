...
@S ngleton
case class PopularV deoSoc alContextBu lder @ nject() (
  ...
  pr vate val popularV deoStr ng = externalStr ngs.soc alContextPopularV deoStr ng
  ...
  def apply(
    ...
    val suggestTypeOpt = cand dateFeatures.getOrElse(SuggestTypeFeature, None)
     f (suggestTypeOpt.conta ns(st.SuggestType. d aT et)) {
      So (
        GeneralContext(
          contextType = SparkleGeneralContextType,
          text = str ngCenter.prepare(popularV deoStr ng),
          url = None,

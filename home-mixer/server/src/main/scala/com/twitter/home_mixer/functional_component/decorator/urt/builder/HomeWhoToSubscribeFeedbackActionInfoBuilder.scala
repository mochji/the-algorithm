...
object Ho WhoToSubscr beFeedbackAct on nfoBu lder {
  pr vate val Feedback tadata = tl.Feedback tadata(
     nject onType = So (SuggestType.WhoToSubscr be),
    engage ntType = None,
    ent y ds = Seq.empty,
    ttlMs = None
  )
...
@S ngleton
case class Ho WhoToSubscr beFeedbackAct on nfoBu lder @ nject() (
  ...
  pr vate val whoToSubscr beFeedbackAct on nfoBu lder = WhoToFollowFeedbackAct on nfoBu lder(
    ...
    encodedFeedbackRequest =
      So (Ho WhoToSubscr beFeedbackAct on nfoBu lder.EncodedFeedbackRequest)
  )
  ...
  overr de def apply(
    ...
    whoToSubscr beFeedbackAct on nfoBu lder.apply(query, cand date, cand dateFeatures)
}

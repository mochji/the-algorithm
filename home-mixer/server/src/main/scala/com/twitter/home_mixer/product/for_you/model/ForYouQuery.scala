...
case class For Query(
  ...
  overr de def w hFeatureMap(features: FeatureMap): For Query =
    copy(features = So (features))

  overr de val t  l neRequestParams: Opt on[T  l neRequestParams] =
    So (T  l neRequestParams(ho T  l neType = So (Ho T  l neType.Ho )))

  // F elds below are used for FL P  nject on  n Onboard ng Task Serv ce (OTS)
  overr de val d splayLocat on: ots.D splayLocat on = ots.D splayLocat on.Ho T  l ne

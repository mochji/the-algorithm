package com.tw ter.ho _m xer.product.follow ng
 ...
 mport com.tw ter.ho _m xer.product.follow ng.param.Follow ngParam.EnableFl p nject onModuleCand dateP pel neParam
 mport com.tw ter.ho _m xer.product.follow ng.param.Follow ngParam.Fl p nl ne nject onModulePos  on
 mport com.tw ter.ho _m xer.product.follow ng.param.Follow ngParam.ServerMaxResultsParam
 mport com.tw ter.ho _m xer.product.follow ng.param.Follow ngParam.WhoToFollowPos  onParam
 ...
 mport com.tw ter.product_m xer.component_l brary.feature_hydrator.query.param_gated.AsyncParamGatedQueryFeatureHydrator
 mport com.tw ter.product_m xer.component_l brary.gate.NonEmptyCand datesGate
 mport com.tw ter.product_m xer.component_l brary.model.cand date.T etCand date
 mport com.tw ter.product_m xer.component_l brary.p pel ne.cand date.flex ble_ nject on_p pel ne.Fl pPromptDependentCand dateP pel neConf gBu lder
...
@S ngleton
class Follow ngM xerP pel neConf g @ nject() (
  follow ngEarlyb rdCand dateP pel neConf g: Follow ngEarlyb rdCand dateP pel neConf g,
  ...
  follow ngAdsCand dateP pel neBu lder: Follow ngAdsCand dateP pel neBu lder,
  follow ngWhoToFollowCand dateP pel neConf gBu lder: Follow ngWhoToFollowCand dateP pel neConf gBu lder,
  fl pPromptDependentCand dateP pel neConf gBu lder: Fl pPromptDependentCand dateP pel neConf gBu lder,
  ed edT etsCand dateP pel neConf g: Ed edT etsCand dateP pel neConf g,
  newT etsP llCand dateP pel neConf g: NewT etsP llCand dateP pel neConf g[Follow ngQuery],
  ...
  realGraph nNetworkS ceQueryHydrator: RealGraph nNetworkScoresQueryFeatureHydrator,
  requestQueryFeatureHydrator: RequestQueryFeatureHydrator[Follow ngQuery],
  ...
  lastNonPoll ngT  QueryFeatureHydrator: LastNonPoll ngT  QueryFeatureHydrator,
  ads njector: Ads njector,
  updateLastNonPoll ngT  S deEffect: UpdateLastNonPoll ngT  S deEffect[Follow ngQuery, T  l ne],
  ...
  @Flag(Scr beCl entEventsFlag) enableScr beCl entEvents: Boolean)
    extends M xerP pel neConf g[Follow ngQuery, T  l ne, urt.T  l neResponse] {

  overr de val  dent f er: M xerP pel ne dent f er = M xerP pel ne dent f er("Follow ng")
  ...
  overr de val fetchQueryFeatures: Seq[QueryFeatureHydrator[Follow ngQuery]] = Seq(
    requestQueryFeatureHydrator,
    sgsFollo dUsersQueryFeatureHydrator,
    realGraph nNetworkS ceQueryHydrator,
    ...
    AsyncQueryFeatureHydrator(dependentCand datesStep, pers stenceStoreQueryFeatureHydrator),
    AsyncQueryFeatureHydrator(dependentCand datesStep, lastNonPoll ngT  QueryFeatureHydrator),
  ...
  pr vate val earlyb rdCand dateP pel neScope =
    Spec f cP pel ne(follow ngEarlyb rdCand dateP pel neConf g. dent f er)
  ...
  pr vate val follow ngAdsCand dateP pel neConf g =
    follow ngAdsCand dateP pel neBu lder.bu ld(earlyb rdCand dateP pel neScope)

  pr vate val follow ngWhoToFollowCand dateP pel neConf g =
    follow ngWhoToFollowCand dateP pel neConf gBu lder.bu ld(earlyb rdCand dateP pel neScope)

  pr vate val fl pPromptCand dateP pel neConf g =
    fl pPromptDependentCand dateP pel neConf gBu lder.bu ld[Follow ngQuery](
      supportedCl entParam = So (EnableFl p nject onModuleCand dateP pel neParam)
    )

  overr de val cand dateP pel nes: Seq[Cand dateP pel neConf g[Follow ngQuery, _, _, _]] =
    Seq(follow ngEarlyb rdCand dateP pel neConf g)

  overr de val dependentCand dateP pel nes: Seq[
    DependentCand dateP pel neConf g[Follow ngQuery, _, _, _]
  ] = Seq(
    conversat onServ ceCand dateP pel neConf g,
    follow ngAdsCand dateP pel neConf g,
    follow ngWhoToFollowCand dateP pel neConf g,
  ...
  overr de val fa lOpenPol c es: Map[Cand dateP pel ne dent f er, Fa lOpenPol cy] = Map(
    follow ngAdsCand dateP pel neConf g. dent f er -> Fa lOpenPol cy.Always,
    follow ngWhoToFollowCand dateP pel neConf g. dent f er -> Fa lOpenPol cy.Always,
  ...
  overr de val resultSelectors: Seq[Selector[Follow ngQuery]] = Seq(
    ...
    DropModuleTooFewModule emResults(
      cand dateP pel ne = follow ngWhoToFollowCand dateP pel neConf g. dent f er,
      m nModule emsParam = Stat cParam(WhoToFollowArmCand dateP pel neConf g.M nCand datesS ze)
    ),
    DropMaxModule emCand dates(
      cand dateP pel ne = follow ngWhoToFollowCand dateP pel neConf g. dent f er,
      maxModule emsParam = Stat cParam(WhoToFollowArmCand dateP pel neConf g.MaxCand datesS ze)
    ),
     ...
     nsertAdResults(
      ...
      adsCand dateP pel ne = follow ngAdsCand dateP pel neConf g. dent f er
    ),
  ...
  pr vate val ho Scr beCl entEventS deEffect = Ho Scr beCl entEventS deEffect(
    ...
    adsCand dateP pel ne dent f er = So (follow ngAdsCand dateP pel neConf g. dent f er),
    whoToFollowCand dateP pel ne dent f er =
      So (follow ngWhoToFollowCand dateP pel neConf g. dent f er),
  )

  overr de val resultS deEffects: Seq[P pel neResultS deEffect[Follow ngQuery, T  l ne]] = Seq(
  ...
  overr de val doma nMarshaller: Doma nMarshaller[Follow ngQuery, T  l ne] = {
    ...
    val  tadataBu lder = Urt tadataBu lder(
      ...
      scr beConf gBu lder = So (
        Stat cT  l neScr beConf gBu lder(
          T  l neScr beConf g(page = So ("follow ng"), sect on = None, ent yToken = None)))
    )

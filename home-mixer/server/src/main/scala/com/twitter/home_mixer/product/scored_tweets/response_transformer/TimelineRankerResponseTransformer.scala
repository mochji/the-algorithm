...
object T  l neRankerResponseTransfor r {
  ...
  def transform(cand date: tlr.Cand dateT et): FeatureMap = {
    val t et = cand date.t et
    val quotedT et = t et.f lter(_.quotedT et.ex sts(_.t et d != 0)).flatMap(_.quotedT et)
    ...
    FeatureMapBu lder()
      ...
      .add(Earlyb rdFeature, cand date.features)
      .add(Earlyb rdScoreFeature, cand date.features.map(_.earlyb rdScore))
      .add(
        Exclus veConversat onAuthor dFeature,
      ...
      .add( sRandomT etFeature, cand date.features.ex sts(_. sRandomT et.getOrElse(false)))
      ...
      .add(T etUrlsFeature, cand date.features.flatMap(_.urlsL st).getOrElse(Seq.empty))
      .bu ld()
  }
}

...
publ c class Str ctRecencyCac F lter extends
    Cac F lter<Earlyb rdRequestContext, Earlyb rdRequest, Earlyb rdResponse> {
  /**
   * Creates a cac  f lter for earlyb rd str ct recency requests.
   */
  @ nject
  publ c Str ctRecencyCac F lter(
      @Str ctRecencyCac  Cac <Earlyb rdRequest, Earlyb rdResponse> cac ,
    ...
    super(cac ,
          new Str ctRecencyQueryCac Pred cate(dec der, normal zedSearchRootNa ),
          ...
          new Earlyb rdRequestPerCl entCac Stats(
              Earlyb rdRequestType.STR CT_RECENCY.getNormal zedNa ()));
  }
}

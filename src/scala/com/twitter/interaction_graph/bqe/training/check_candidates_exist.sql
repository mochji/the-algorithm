...
SET date_end = (SELECT DATE(T MESTAMP_M LL S($start_t  $)));
SET date_start = DATE_SUB(date_end,  NTERVAL 30 DAY);

CREATE OR REPLACE TABLE `twttr-recos-ml-prod.realgraph.cand dates_for_tra n ng` 
PART T ON BY ds
AS
W TH T1 AS (
...
LEFT JO N `twttr-bq-cassowary-prod.user. nteract on_graph_agg_negat ve_edge_snapshot` N
US NG (s ce_ d, dest nat on_ d)
...
SELECT ds FROM `twttr-recos-ml-prod.realgraph.cand dates_for_tra n ng`
WHERE ds = (SELECT DATE(T MESTAMP_M LL S($start_t  $)))
L M T 1

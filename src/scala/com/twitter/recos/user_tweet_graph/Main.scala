package com.tw ter.recos.user_t et_graph
 ...
 mport com.tw ter.recos.user_t et_graph.RecosConf g._
 ...
 mport com.tw ter.recos.user_t et_graph.store.UserRecentFollo rsStore
...
object Ma n extends Tw terServer w h JDK14Logg ng w h Dec derable {
  ...
  def ma n(): Un  = {
    ...
    val kafkaConf gBu lder = F nagleKafkaConsu rBu lder[Str ng, RecosHose ssage]()
      ...
      .group d(KafkaGroup d(f"user_t et_graph-${shard d()}%06d"))
    ...
    val userT etGraph =
      new UserT etGraph(
        ...
        endpo ntLoadS dder)(t  r)

    val thr ftServer = Thr ftMux.server
      .w hOpportun st cTls(Opportun st cTls.Requ red)
      .w hMutualTls(serv ce dent f er)
      .serve face(serv cePort(), userT etGraph)
